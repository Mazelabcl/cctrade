...
def get_market_status(session):
    """Get current market status: price, nearest levels, distances, open trades.

    Returns dict with all info needed for smart Telegram alerts.
    """
    from ..services.level_trade_backtest_db import load_levels_db
    ...
    if not latest:
        return {'status': 'no_data', 'message': 'No candles available'}
    ...
    levels = (
        ...
        .filter(
            Level.invalidated_at.is_(None),
            Level.created_at <= latest.open_time,
        )
        .all()
    )

    # Filter to naked structural + current mobile
    naked_levels = []
    for l in levels:
        is_mobile = l.level_type.startswith('PrevSession') or l.level_type.startswith('VP_')
        if is_mobile:
            naked_levels.append(l)  # Mobile levels handled by superseded_at
        elif l.first_touched_at is None:
            naked_levels.append(l)  # Structural naked

    # Find nearest support and resistance
    supports = []
    resistances = []
    for l in naked_levels:
        if l.price_level <= price:
            dist_pct = (price - l.price_level) / price * 100
            supports.append({
                'price': l.price_level,
                'type': l.level_type,
                'tf': l.timeframe,
            ...
            dist_pct = (l.price_level - price) / price * 100
            resistances.append({
                'price': l.price_level,
                'type': l.level_type,
                'tf': l.timeframe,
    ...
    # Check confluence zones near support/resistance
    zone_width = config['zone_width']
    approaching_zones = []

    for zone_center in (supports[:3] + resistances[:3]):
        zone_lo = zone_center['price'] * (1 - zone_width)
        zone_hi = zone_center['price'] * (1 + zone_width)
        zone_levels = [l for l in naked_levels if zone_lo <= l.price_level <= zone_hi]
        zone_types = set(l.level_type for l in zone_levels)

        if len(zone_types) >= 2 and zone_center['dist_pct'] < 1.5:
            zone_detail = [{'type': l.level_type, 'tf': l.timeframe,
                           'price': round(l.price_level, 2)} for l in zone_levels[:8]]
            approaching_zones.append({
                'center_price': round(zone_center['price'], 2),
                'dist_pct': zone_center['dist_pct'],
                'direction': 'support' if zone_center['price'] <= price else 'resistance',
                'confluence': len(zone_types),
                'levels': zone_detail,
            })
    ...
    for t in open_trades:
        unrealized_r = t.compute_pnl(price)
        open_trade_info.append({
            ...
            'unrealized_r': round(unrealized_r, 2) if unrealized_r else 0,
        })

    return {
        'status': 'approaching' if approaching_zones else 'watching',
        'price': round(price, 2),
        'candle_time': latest.open_time.isoformat() if latest.open_time else None,
        'nearest_support': supports[0] if supports else None,
        'nearest_resistance': resistances[0] if resistances else None,
        'approaching_zones': approaching_zones,
        'open_trades': open_trade_info,
    }
